...
DATA_DIR = r"c:\Users\birge\OneDrive - NTNU\Skole\Master\VSCode\testdata\lekkasjetest_1_liten_finger"
TARGET_PRESSURE = 0.03
STAT_WINDOW = 10
SMOOTH_WINDOW = 15
HOLD_DURATION = 55.0
...
for i, fname in enumerate(files):
    ...
    stats.append((f"Run {i + 1}", begin_p, end_p, drop, rate, rel))

fig, ax = plt.subplots(figsize=(9, 5))

for j, curve in enumerate(interp_curves):
    ax.plot(COMMON_T, curve, color=colors[j], linewidth=2.0, label=f"Run {j + 1}")
...
plt.tight_layout()
plt.savefig(os.path.join(DATA_DIR, "litenfinger_smoothedlekkasjetest_1finger_0.03bar60s.png"), dpi=150)
...
header = f"{'Run':<10} {'Begin [bar]':>14} {'End [bar]':>12} {'Drop [bar]':>12} {'Rate [bar/s]':>14} {'Rel [%/s]':>12}"
print(header)
print("-" * len(header))
for s in rows:
    name, bp, ep, drop, rate, rel = s
