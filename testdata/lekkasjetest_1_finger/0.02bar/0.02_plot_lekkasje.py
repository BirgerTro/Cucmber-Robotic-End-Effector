...
DATA_DIR = r"c:\Users\birge\OneDrive - NTNU\Skole\Master\VSCode\testdata\lekkasjetest_1_finger\0.02bar"
TARGET_PRESSURE = 0.02
STAT_WINDOW = 10
SMOOTH_WINDOW = 15
HOLD_DURATION = 55.0  # seconds to show after target is reached
COMMON_T = np.linspace(0, HOLD_DURATION, 1200)  # common time grid for interpolation
...
# ── Collect normalised, smoothed curves ──────────────────────────────────────
interp_curves = []
stats = []
...
for i, fname in enumerate(files):
    ...
    first_idx = above[0]
    t_norm = times - times[first_idx]          # normalise: t=0 at target
...
plt.tight_layout()
plt.savefig(os.path.join(DATA_DIR, "lekkasje_0.02bar_1finger.png"), dpi=150)

# ── Statistics table ──────────────────────────────────────────────────────────
mean_vals = [float(np.mean([s[j] for s in stats])) for j in range(1, 6)]
std_vals  = [float(np.std([s[j]  for s in stats])) for j in range(1, 6)]
stats.append(("Mean", *mean_vals))
stats.append(("Std",  *std_vals))
